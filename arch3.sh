...
if [[ $driver == 1 ]]; then
  ...
  echo '#!/bin/bash
  sleep 5
  sudo systemctl restart vmtoolsd.service' > ~/.config/autostart-scripts/restart && sudo chmod +x ~/.config/autostart-scripts/restart      #config/autostart-scripts/restart
...
yay -S --mflags --skipinteg --noconfirm archiso netactview loudmouth hddtemp plasma5-applets-systemd plasma5-applets-thermal-monitor plasma5-applets-eventcalendar netdiscover fail2ban plasma5-applets-netspeed ffmulticonverter steghide
